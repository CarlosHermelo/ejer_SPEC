...
|---|----------|-----------------|-------------|-----------|
| 1 | `requests` está en `requirements.txt` | `grep requests backend/requirements.txt` | [AGENTE] | Pass |
| 2 | `.env.example` documenta las 3 variables | Verificar presencia de `KAPSO_API_KEY`, `PHONE_NUMBER_ID`, `NUMERO_DESTINO` | [AGENTE] | Pass |
| 3 | El script existe en la ruta correcta | `ls backend/scripts/send_whatsapp.py` | [AGENTE] | Pass |
| 4 | Sin argumento, el script sale con error | `python backend/scripts/send_whatsapp.py` → error en stderr y exit code 1 | [AGENTE] | Pass |
